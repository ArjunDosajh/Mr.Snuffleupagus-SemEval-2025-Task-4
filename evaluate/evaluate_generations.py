...
def get_args_and_verify():
    ...
    parser.add_argument('--batch_size', required=False, type=int, default=2, help='Batch size for inference')
    ...
    parser.add_argument('--alphas', required=True, default=None, help='Alphas for MIA attack')
    parser.add_argument('--layer_ids', required=True, default=None, help='Layer IDs for MIA attack')
    # parser.add_argument('--shuffle', required=True, default=None, help='Shuffle for MIA attack')
...
#        aggregate_scores_list.append(mmlu_scores['average_acc'])
    ...
    alpha_str = '_'.join(str(int(float(x))) for x in args.alphas.split(','))
    layer_ids_str = '_'.join(str(int(float(x))) for x in args.layer_ids.split(','))
       
    metrics_file = os.path.join(args.output_dir, f'olmo1b_alpha-{alpha_str}_layerIds-{layer_ids_str}_evaluation_result.json')
